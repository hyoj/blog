...
; (def s "<ol class=\"content\"><li><a href=\"#_example_asciidoc_post\">Example Asciidoc Post</a></li><ol><li><a href=\"#_section_1\">Section 1</a></li><li><a href=\"#_section_2\">Section 2</a></li></ol></ol>")
; (change-all-tags {:tag :ol :content [{:tag :ol :content nil}]} :ol :ul)
; (require '[hickory.core :as hic])
; (require '[hickory.render :as r])

(defn change-tag [elem from-tag to-tag]
  (if (and (map? elem)
       (= (:tag elem) from-tag))
    (assoc elem :tag to-tag)
    elem))

(defn change-all-tags [elem from-tag to-tag]
  (if (and elem (map? elem))
    (let [elem (change-tag elem from-tag to-tag)
          content (when-let [content (:content elem)]
                    (into [] (map #(change-all-tags % from-tag to-tag) content)))]
      (assoc elem :content content))
    elem))

(defn to-hickory [s]
  (map hic/as-hickory (hic/parse-fragment s)))

(defn change [s]
  (when s
    (-> (first (to-hickory s))
        (change-all-tags :ol :ul)
        hickory-to-html)))
...
(defn compile-posts
  ...
  (when-not (empty? posts)
    ...
    (doseq [{:keys [uri] :as post} posts]
      (let [post (assoc post :nav (change (:toc post)))]
        (println "\n===========\n" (:nav post) "\n===========\n")
        (println "-->" (cyan uri))
        (when debug?
